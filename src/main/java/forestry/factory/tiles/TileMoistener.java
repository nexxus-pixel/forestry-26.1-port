...
public class TileMoistener extends TileBase implements WorldlyContainer, ILiquidTankTile, IRenderableTile {
	...
	protected void writeLegacyData(CompoundTag compoundNBT) {
		...
		if (this.pendingProduct != null) {
            ...
            ItemStackUtil.saveToTag(pendingProduct, CompoundNBTP);
			compoundNBT.put("PendingProduct", CompoundNBTP);
		}
		if (this.currentProduct != null) {
			CompoundTag CompoundNBTP = new CompoundTag();
            ItemStackUtil.saveToTag(currentProduct, CompoundNBTP);
	...
	protected void readLegacyData(CompoundTag compoundNBT) {
        ...
        this.burnTime = CompoundTagUtil.getInt(compoundNBT, "BurnTime");
        this.totalTime = CompoundTagUtil.getInt(compoundNBT, "TotalTime");
        this.productionTime = CompoundTagUtil.getInt(compoundNBT, "ProductionTime");

        this.tankManager.read(compoundNBT);

		// Load pending product
		if (compoundNBT.contains("PendingProduct")) {
			CompoundTag compoundNBTP = CompoundTagUtil.getCompound(compoundNBT, "PendingProduct");
            this.pendingProduct = ItemStackUtil.loadFromTag(compoundNBTP);
		}
		if (compoundNBT.contains("CurrentProduct")) {
			CompoundTag compoundNBTP = CompoundTagUtil.getCompound(compoundNBT, "CurrentProduct");
            this.currentProduct = ItemStackUtil.loadFromTag(compoundNBTP);
		}
	...
	public boolean hasFuelMin(float percentage) {
		...
		for (int i = InventoryMoistener.SLOT_STASH_1; i < InventoryMoistener.SLOT_RESERVOIR_1; i++) {
			...
			if (FuelManager.moistenerResource.containsKey(inventory.getItem(i))) {
				...
				if (inventory.getItem(i).is(res.resource())) {
					max += 64;
