...
public class TradeStation implements ITradeStation {
	...
	public void read(CompoundTag nbt) {
		if (nbt.contains("owner")) {
            this.owner = GameProfileUtil.read(CompoundTagUtil.getCompound(nbt, "owner"));
		}

		if (nbt.contains("address")) {
            this.address = new MailAddress(CompoundTagUtil.getCompound(nbt, "address"));
		}

		this.isVirtual = CompoundTagUtil.getBoolean(nbt, "VRT");
		this.isInvalid = CompoundTagUtil.getBoolean(nbt, "IVL");
	...
	public IPostalState handleLetter(ServerLevel world, IMailAddress recipient, ItemStack letterstack, boolean doLodge) {
		...
		ItemStackUtil.setTag(mailstack, compoundNBT);
		...
		if (sendOwnerNotice) {
			...
			ItemStackUtil.setTag(confirmstack, compoundNBT);
	...
	private int countFillableOrders(int max, ItemStack tradegood) {
		...
		for (ItemStack stack : InventoryUtil.getStacks(this.inventory, SLOT_SEND_BUFFER, SLOT_SEND_BUFFER_COUNT)) {
			if (stack != null && ItemStackUtil.isSameItemSameTags(stack, tradegood)) {
	...
	private void removeTradegood(int filled) {
		for (int j = 0; j < filled; j++) {
			...
			for (int i = SLOT_SEND_BUFFER; i < SLOT_SEND_BUFFER + SLOT_SEND_BUFFER_COUNT; i++) {
				...
				if (!buffer.isEmpty() && ItemStackUtil.isSameItemSameTags(buffer, this.inventory.getItem(SLOT_TRADEGOOD))) {
