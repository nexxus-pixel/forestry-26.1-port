...
public final class FarmType implements IFarmType {
	...
	public FarmType(Identifier id, Item icon, BiFunction<IFarmType, Boolean, IFarmLogic> logicFactory, ImmutableList<IFarmable> farmables, ToIntFunction<IFarmHousing> fertilizerConsumption, IWaterConsumption waterConsumption, ImmutableSet<Soil> soils) {
		this.id = id;
		this.icon = icon;
		this.soils = soils;
	...
	public boolean isAcceptedResource(ItemStack stack) {
		for (Soil soil : this.soils) {
			if (ItemStack.isSameItem(soil.resourceStack(), stack)) {
				return true;
			}
		}
		return false;
