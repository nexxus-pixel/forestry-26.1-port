...
public class BlockForestryTrapdoor extends TrapDoorBlock implements IWoodTyped {
	...
	public BlockForestryTrapdoor(ForestryWoodType type) {
		super(type.getBlockSetType(), RegistrationContext.of(p -> p.mapColor(MapColor.WOOD).instrument(NoteBlockInstrument.BASS).strength(3f).noOcclusion().isValidSpawn(BlockUtil.NEVER_SPAWN).ignitedByLava()));
