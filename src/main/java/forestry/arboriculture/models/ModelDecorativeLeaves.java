...
public class ModelDecorativeLeaves<B extends Block> extends ModelBlockCached<B, ModelDefaultLeaves.Key> {
	...
	protected void bakeBlock(B block, ModelData extraData, ModelDefaultLeaves.Key key, ModelBaker baker, boolean inventory) {
		Identifier speciesId = key.speciesId;
		...
		Identifier textureLocation = sprite.get(false, key.fancy);
		...
		Identifier fruitSpriteLocation = species.getDefaultGenome().getActiveValue(TreeChromosomes.FRUIT).getDecorativeSprite();
		...
		Identifier particleLocation = sprite.getParticle();
