...
public abstract class MultiblockControllerForestry extends MultiblockControllerBase implements WorldlyContainer, IOwnedTile, IErrorLogicSource, ILocationProvider {
	...
	@Override
	public final void startOpen(ContainerUser user) {
		getInternalInventory().startOpen(user);
	}

	@Override
	public final void stopOpen(ContainerUser user) {
		getInternalInventory().stopOpen(user);
	}
