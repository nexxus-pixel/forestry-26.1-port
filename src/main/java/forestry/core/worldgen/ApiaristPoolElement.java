...
public class ApiaristPoolElement extends SinglePoolElement {
	...
	public ApiaristPoolElement(Either<Identifier, StructureTemplate> template, Holder<StructureProcessorList> processors) {
	...
	public void handleDataMarker(LevelAccessor level, StructureTemplate.StructureBlockInfo info, BlockPos pos, Rotation rotation, RandomSource random, BoundingBox box) {
		...
		if (info.nbt() != null) {
			if (StructureMode.valueOf(CompoundTagUtil.getString(info.nbt(), "mode")) == StructureMode.DATA) {
				String marker = CompoundTagUtil.getString(info.nbt(), "metadata");
