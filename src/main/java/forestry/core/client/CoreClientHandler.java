...
public class CoreClientHandler implements IClientModuleHandler {
	...
	public void registerEvents(IEventBus modBus) {
		...
		ModuleUtil.getModBus(ForestryConstants.MOD_ID).addListener(Priority.HIGHEST, ((ForestryClientApiImpl) IForestryClientApi.INSTANCE)::initializeTextureManager);
	}
	...
	private static void additionalBakedModels(ModelEvent.RegisterAdditional event) {
		...
		for (BeeLifeStage stage : BeeLifeStage.values()) {
			Map<IBeeSpecies, Identifier> models = beeManager.getBeeModels(stage);
		...
		for (Pair<Identifier, Identifier> pair : treeManager.getAllSaplingModels()) {
	...
	private static void registerReloadListeners(RegisterClientReloadListenersEvent event) {
		...
		event.registerReloadListener((state, backgroundExecutor, prepBarrier, gameExecutor) -> prepBarrier.wait(null).thenRunAsync(ModelBlockCached::clear, gameExecutor));
	}
