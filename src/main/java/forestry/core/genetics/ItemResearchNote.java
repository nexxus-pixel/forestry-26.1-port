...
public class ItemResearchNote extends ItemForestry {
	...
	public Component getName(ItemStack itemstack) {
		ResearchNote note = new ResearchNote(ItemStackUtil.getTag(itemstack));
		String researcherName;
		if (note.researcher == null) {
			researcherName = "Sengir";
		} else {
			researcherName = note.researcher.name();
		}
	...
	public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
		...
		ResearchNote note = new ResearchNote(ItemStackUtil.getTag(heldItem));
	...
	private static IMutation<?> getEncodedMutation(CompoundTag compound) {
		ISpeciesType<?, ?> type = IForestryApi.INSTANCE.getGeneticManager().getSpeciesTypeSafe(Identifier.parse(compound.getString(TYPE_KEY)));
		if (type == null) {
			return null;
		}
	...
	public static ItemStack createMutationNoteStack(GameProfile researcher, IMutation<?> mutation) {
		...
		ItemStackUtil.setTag(created, compound);
		return created;
	}
	...
	public static class ResearchNote {
		...
		public ResearchNote(@Nullable CompoundTag nbt) {
			if (nbt != null) {
				if (nbt.contains(RESEARCHER_KEY)) {
					this.researcher = GameProfileUtil.read(CompoundTagUtil.getCompound(nbt, RESEARCHER_KEY));
				} else {
		...
		public CompoundTag writeToNBT(CompoundTag compound) {
			if (this.researcher != null) {
				...
				GameProfileUtil.write(nbt, this.researcher);
