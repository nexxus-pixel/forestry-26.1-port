...
public class FilterLogic implements IFilterLogic {
	...
	public void read(CompoundTag data) {
		for (int i = 0; i < this.filterRules.length; i++) {
			this.filterRules[i] = IForestryApi.INSTANCE.getFilterManager().getRuleOrDefault(CompoundTagUtil.getString(data, "TypeFilter" + i));
		}
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 3; j++) {
				...
				if (data.contains("GenomeFilterS" + i + "-" + j + "-" + 0)) {
					filter.activeSpecies = SpeciesUtil.getAnySpecies(Identifier.parse(CompoundTagUtil.getString(data, "GenomeFilterS" + i + "-" + j + "-" + 0)));
				}
				if (data.contains("GenomeFilterS" + i + "-" + j + "-" + 1)) {
					filter.inactiveSpecies = SpeciesUtil.getAnySpecies(Identifier.parse(CompoundTagUtil.getString(data, "GenomeFilterS" + i + "-" + j + "-" + 1)));
				}
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 3; j++) {
				...
				if (filter.activeSpecies != null) {
					...
					buffer.writeIdentifier(filter.activeSpecies.id());
				} else {
					buffer.writeBoolean(false);
				}
				if (filter.inactiveSpecies != null) {
					buffer.writeBoolean(true);
					buffer.writeIdentifier(filter.inactiveSpecies.id());
				} else {
					buffer.writeBoolean(false);
				}
			}
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 3; j++) {
				...
				if (buffer.readBoolean()) {
					filter.activeSpecies = SpeciesUtil.getAnySpecies(buffer.readIdentifier());
				}
				if (buffer.readBoolean()) {
					filter.inactiveSpecies = SpeciesUtil.getAnySpecies(buffer.readIdentifier());
				}
